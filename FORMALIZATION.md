...
## Status vocabulary
- **VERIFIED** — machine-checked in `Verified/`, axiom-clean: `#print axioms`
...
lake env lean Verified/AxiomCheck.lean   # prints #print axioms for all 36 results
```
Last clean re-elaboration (oleans absent → full re-elaboration of `Verified/`):
...
- **`prop:cv2` coefficient** — the polynomial moments `E[u]=1/(n+1)`, `E[u²]=2/((n+1)(n+2))` are
  now **VERIFIED** (`Verified/BetaMoments.lean`, explicit antiderivatives + FTC; no
...
- **`thm:transfer` plateau half — slowly varying** — the generalization to `N(λ)=λ^{d/2}·ℓ(λ)`
  ...
  in Mathlib. The *exact power-law* case is now **VERIFIED** (`Verified/PowerLaw.lean`); only the
...
---
*Frontier (both landed):* **B2** — exact power-law plateau VERIFIED (`Verified/PowerLaw.lean`,
...
`E[u²]` VERIFIED (`Verified/BetaMoments.lean`, 4 theorems, explicit antiderivatives + FTC); since
